...
function findEdgeOnRay(imageData, startPoint, endPoint) {
  ...
  let start = null, end = null, edgePixel = null;

  enrichedPixels.forEach((pixel, index) => {
    if (pixel.color.isOpaque()) {
      if (start === null) { start = index; }
      else { end = index; }
      edgePixel = !edgePixel || edgePixel.color.alpha < pixel.color.alpha ? pixel : edgePixel;
    } else if (end !== null) {
      start = null; end = null;
    }
  });
...
function createRays(mask, maskImageData) {
  const maskCentre = { x: mask.width / 2, y: mask.height / 2 };
  ...
  for (let rayIndex = 0; edgePoints > rayIndex; rayIndex++) {
    ...
    const ray = createRay(maskImageData, maskCentre, getCanvasEdge(mask, rayIndex), sampleRay);
    if (sampleRay && !ray.edgePixel) {
      for (let stepIndex = rayIndex - 1; stepIndex < CONSTANTS.MASK.SAMPLE_SIZE && (rayIndex - stepIndex) >= 0; stepIndex++) {
        const stepRay = createRay(maskImageData, maskCentre, getCanvasEdge(mask, stepIndex), sampleRay);
        if (stepRay.edgePixel) { rays.edgePixel = stepRay.edgePixel; break; }
      }
    }
    rays.push(ray);
  }
...
export function generateRayMask(maskCanvas) {
  ...
  const edgePoints = rays.filter((ray) => ray.edgePixel).map((ray) => ray.edgePixel);
  ...
  if (edgePoints.length < 2) {
    ...
    edgePoints.forEach((edgePoint, index) => {
      if (index === 0) context.moveTo(edgePoint.x, edgePoint.y);
      else context.lineTo(edgePoint.x, edgePoint.y);
    });
    context.closePath();
    context.fill();
  }
