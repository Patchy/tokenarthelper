...
export default class View {
  constructor(editor, dimension, element) {
    this.editor = editor;
    ...
    if (element.id === "arteditor-token") {
      this.stage.setAttribute("id", "token-canvas");
      this.type = "token";
    } else if (element.id === "arteditor-portrait") {
      this.stage.setAttribute("id", "portrait-canvas");
      this.type = "portrait";
    }
  ...
  initializeMenu() {
    ...
    title.innerHTML = game.i18n.localize("tokenarthelper.label.NewImage");
  ...
  #addLayerControls(layer, { masked, activate } = {}) {
    ...
    try {
      const control = new Control(this.editor, layer, this.layers.length - 1);
  ...
  /**
   * Ends an alpha picking state
   * @param {boolean} reset If the user aborted the alpha picking, we will reset to the original alphas
   */
  ...
  /**
   * Deactivates all layers (can only be one active at a time...)
   */
  deactivateLayers() {
    this.activeLayer = null;
    this.layers.forEach((layer) => (layer.active = false));
