...
export class MagicLasso {
  ...
  async display(callback, nestedCallback) {
    const html = await foundry.applications.handlebars.renderTemplate("modules/tokenarthelper/templates/magic-lasso.hbs");
  ...
  #activateListeners(callback, nestedCallback) {
    ...
    const blurRadius = document.getElementById("arteditor-blur-radius");
  ...
  showThreshold() {
    document.getElementById("arteditor-threshold").innerHTML = `Threshold: ${this.currentThreshold}`;
  }

  showBlur() {
    document.getElementById("arteditor-blur-radius").value = this.blurRadius;
  }
